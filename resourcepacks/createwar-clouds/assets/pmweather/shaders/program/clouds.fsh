...
// =============================================================================
// CREATEWAR PERFORMANCE PATCHES (set any of these to 0 to disable individually)
// =============================================================================
// Patch 1: Pre-cull storms by XZ distance before reading their per-storm uniforms.
//          Squall storms have the largest effective influence radius (~stormSize*45),
//          so we use stormSize*100 + 5000 as a conservative cap.
#define CREATEWAR_STORM_DIST_CULL 1

// Patch 2: After several consecutive clear-air samples, accelerate step size to
//          cover empty space faster. Safe because cloud densities below 0.1 would
//          already be discarded by step(0.1, rd).
#define CREATEWAR_FAST_CLEAR_STEP 1

// Patch 3: Reduce noise octaves for samples far from camera. Distant clouds
//          can't show fine detail anyway. 0 octaves cut at 0-2000 blocks, then
//          progressively more, capped at 3.
#define CREATEWAR_DISTANCE_OCTAVE_LOD 1
// =============================================================================
...
uniform float rainIntensities[16];
...
uniform float computationalFBMNoise;
...
float tfbm(vec3 x, int octaves, float lacunarity, float gain, float amplitude){
    float y = 0.0;
...
float fbm(vec3 x, int octaves, float lacunarity, float gain, float amplitude){
    if(computationalFBMNoise > 0.5){
        return cfbm(x, octaves, lacunarity, gain, amplitude);
    }

    return tfbm(x, octaves, lacunarity, gain, amplitude);
}
...
CloudReturn getClouds(vec3 position, int octaveReduction){
    ...
    vec3 overrideNoisePos = position + vec3(worldTime*0.25, 0, worldTime*0.25);
    ...
    float boverrideNoise = clamp(noise2d(overrideNoisePos.xz/200.0)+1.0, 0.0, 2.0)/2.0;
    ...
    if(position.y > layer0height && position.y < layer0height+1000 && overcastPerc > 0){
        ...
        bgClouds = sqrt(sqrt(sqrt(bgClouds)))*0.5;
    ...
    if(position.y > layerCheight && position.y < layerCheight+2000 && overcastPerc > 0){
        ...
        bgClouds = sqrt(sqrt(bgClouds))*mix(0.35, 0.0, v*v*v*v*v);
    ...
    for(int i = 0; i < stormCount; i++){
...
#if CREATEWAR_STORM_DIST_CULL
        // Cheap XZ-only distance gate before any further uniform reads.
        {
            vec2 dXZ = position.xz - pos.xz;
            float cullR = stormSize * 100.0 + 5000.0;
            if (dot(dXZ, dXZ) > cullR * cullR) continue;
        }
#endif
        ...
        // Fire Whirl
        if(abs(stormType-3) < 0.1){
            float torPerc = clamp(windspeed/55.0, 0.0, 1.0);
            ...
            float wid = (width/5.0) + (20.0*percFnlHeight*percFnlHeight*percFnlHeight*torPerc);
            ...
            float nx = onoise(vec3(pos.xz/500.0, time/200.0))*5.0*ropeMod;
            float nz = onoise(vec3(time/200.0, pos.zx/500.0))*5.0*ropeMod;
            vec3 attachmentPoint = vec3(nx, 0.0, nz);

            float xAdd = onoise(vec3(pos.xz/250.0, (time/200.0)+((position.y*ropeMod)/50.0)))*3.0*ropeMod;
            float zAdd = onoise(vec3((time/200.0)+((position.y*ropeMod)/50.0), pos.zx/250.0))*3.0*ropeMod;
            ...
            tornado *= sqrt(1.0-clamp(localTorPos.y/heightFadeAt, 0.0, 1.0));
            tornado *= 1.0-((1.0-clamp(localTorPos.y/10.0, 0.0, 1.0))*0.3);

            clouds = max(clouds, tornado);
            totalDust = max(totalDust, tornado * 10.0);
            emit = max(emit, pow(1.0-clamp(localTorPos.y / 50.0, 0.0, 1.0), 1.5) * 0.35 * sqrt(sqrt(tornado)));
        }
        ...
        if(abs(stormType-2) < 0.1 && windspeed > 0){
            ...
            if(staticBands < 0){
                ...
                weak += abs(staticBands) * sqrt(1.0-clamp(dist/(sze*0.65), 0.0, 1.0));
            ...
            localCloud = max(sqrt(1.0-clamp(dist/(sze*eyeSize*2.0), 0.0, 1.0))*(abs(eye*0.3)+0.7)*1.4*intensity, localCloud);

            localCloud *= sqrt(1.0-clamp(dist/sze, 0.0, 1.0));
            ...
            if(heightP <= 1.0){
                ...
                rain *= sqrt(sqrt(1.0-clamp(dist/width, 0.0, 1.0)));
                rain *= 1.0 + noise2*0.2;
        ...
        if(abs(stormType-1) < 0.1){
            ...
            if(behind > 0.0){
                ...
                float baseChange = sqrt(clamp((behind-stormSize)/(stormSize*4.0), 0, 1))*300.0;
            ...
            size = mix(size, mix(mix(size, size/2.0, cellular), size, sqrt(1.0-clamp(currentHeight/300.0, 0.0, 1.0))), sqrt(sqrt(heightPerc)));
            ...
            float cloudField = (noise3-clamp(1.0-(smoothStage-1.0), 0.0, 1.0))*clamp(heightFromTop/65.0, 0.0, 1.0)*sqrt(sqrt(1-distPerc));
            ...
            rainam += rain*rainStrength*rainIntensity;
        }
        ...
        if(abs(stormType) < 0.1){
            ...
            if(position.y < baseHeight && smoothStage > 1.75 && position.y > pos.y-50.0 && dist < stormSize){
                spinNoise2 = fbm(spinNoisePos2/80.0, 4-octaveReduction, 4.0, 0.5, 1.0);
            ...
            size = mix(size, mix(mix(size, size/2.0, supercellular), size, sqrt(1-clamp(currentHeight/300, 0.0, 1.0))), sqrt(sqrt(heightPerc)));
            ...
            float distBased = 1.0-distPerc;
            float cloudField = (noise3-clamp(1.0-smoothStage, 0.0, 1.0))*clamp(heightFromTop/65.0, 0.0, 1.0)*sqrt(sqrt(1.0-distPerc));
            ...
            float wallcloudLower = 120.0*sqrt(sqrt(1.0-distPerc))*clamp((smoothStage-2.0)*3.0, 0.0, 1.0);
            float wallcloudNoiseMod = clamp((spinNoise2+1.0)*0.5, 0.0, 1.0);
            wallcloudLower *= mix(0.5, 1.75, wallcloudNoiseMod*wallcloudNoiseMod);
            ...
            if(tornadic && position.y < baseHeight-wallcloudLower && position.y > pos.y-50.0 && dist < max(width*4.5, stormSize/3.0)){
                float tornado = clamp(torPerc/0.15, 0.0, 1.0);
                float percFnlHeight = clamp((position.y-pos.y)/(fnlTop-pos.y), 0.0, 1.0);
                float percCos = (-cos(percFnlHeight*3.141592)+1.0)*0.5;
                ...
                if(position.y < pos.y+20.0+(width/7.5)+25.0){
                    dcNoise1 = sqrt((fbm(torSpinPos3/60.0, 4-octaveReduction, 5.0, 0.5, 8.0) * 0.5) + 0.75) * 0.45;
                }

                float dcPerc = clamp((windspeed-45.0)/30.0, 0.0, 1.0);
                float h = 5.0 + (dcNoise1*dcNoise1*((width/7.5)+25.0));
                float dcTop = pos.y+(max(dcPerc, 0.35)*h);

                if(position.y < dcTop){
                    float percDCHeight = clamp((position.y-(pos.y-10.0))/(dcTop-pos.y), 0.0, 1.0);

                    wid = ((width/2.5) + ((width/2.5)*percFnlHeight*torPerc) + (25.0*(width/200.0))) + (15.0*pow(percDCHeight, 1.5)*pow(dcPerc, 0.75));
                    wid *= mix(1.0, 1.25, dcNoise1);
                    wid *= mix(0.6 + (dcNoise1*0.5), 0.85, clamp(width/500, 0.0, 1.0)*0.9);
                    wid *= 0.75;
                    widPerc = 1-clamp(torDist/wid, 0.0, 1.0);
                    widPerc = sqrt(sqrt(widPerc));
            ...
            rainam += rain*rainStrength*mix(1.0, (occlusion*0.5)+0.5, clamp(smoothStage-2.0, 0.0, 1.0))*rainIntensity;
            ...
            rainam = max(rainam, rain*rainStrength*rainIntensity);
        }
    ...
    float eV = sqrt(sqrt(clamp(cdensity, 0.0, 1.0)));

    return CloudReturn(cdensity, max(totalRain, 0.0), max(totalDust, 0.0), max(upperLevel, 0.0), clamp(emit*eV, 0.0, 1.0));
}
...
Render render(vec2 uv, float maxDepth){
    ...
    float offset = (onoise(vec3(uv*1024.0, 0))*2.0) - 1.0;
    float density = 0;
    float totalRain = 0;
    float rainDampen = 0;
    ...
    int count = 0;
#if CREATEWAR_FAST_CLEAR_STEP
    int consecutiveClear = 0;  // resets when any density is found; grows in empty space
#endif
    ...
    for(int i = 0; i < ms; i++){
        ...
        if(!inCloud){
            stepMult *= 1.5;
#if CREATEWAR_FAST_CLEAR_STEP
            // Established-clear acceleration: once we've gone several samples without finding
            // any density, take much larger steps to cover empty sky faster.
            if (consecutiveClear > 4) {
                stepMult *= 2.0;
            }
            if (consecutiveClear > 12) {
                stepMult *= 1.5;  // even faster after sustained clear (3x total when very clear)
            }
#endif
        }
        ...
        if(d0 < rDist){
...
#if CREATEWAR_DISTANCE_OCTAVE_LOD
            // Drop noise octaves progressively with distance. Far clouds can't show fine
            // detail at low pixel coverage anyway. 0 octaves cut up to 2000 blocks; then +1 per 2000.
            int distOctaveRed = int(clamp(d0 / 2000.0, 0.0, 3.0));
            CloudReturn realClouds = getClouds(p, distOctaveRed);
#else
            CloudReturn realClouds = getClouds(p, 0);
#endif

            float fogE = clamp(fog/0.1, 0.0, 1.0);
            ...
            d *= 1-clamp((d0-(rDist-3000.0))/3000.0, 0.0, 1.0);
            r *= 1-clamp((d0-(rDist-3000.0))/3000.0, 0.0, 1.0);
            ...
            if(rd > 0){
                ...
                vec3 rainColor = vec3(0.051, 0.341, 0.275);
                vec3 cloudColor = mix(mix(vec3(1.0), rainColor, clamp(rain, 0.0, 1.0)), rainColor, rPerc);
                vec3 dustColor = vec3(0.2, 0.125, 0.071);
                float dustP = clamp(sqrt(sqrt(clouds.dustDensity)), 0.0, 1.0);
                dustColor = mix(dustColor*2.5, dustColor, dustP);

                cloudColor = mix(cloudColor, skyColor*0.5, 0.6);
                cloudColor = mix(cloudColor, dustColor, clamp(sqrt(sqrt(sqrt(clouds.dustDensity))), 0.0, 1.0));

                if(clouds.dustDensity <= 0.0){
                    cloudColor = mix(lightingColor*max(light, moonLight), cloudColor, clamp(pow(d, 0.65)+(r*5.0)+snow, 0, 1));
                }

                cloudColor = mix(cloudColor, mix(vec3(0.05), vec3(0.741, 0.345, 0.0), clamp(fire/2.0, 0.0, 1.0)), clamp(ash/1.5, 0.0, 1.0));
                vec4 col = vec4(mix(cloudColor, mix(vec3(0.145, 0.184, 0.302), rainColor, rPerc), clamp(rd, 0.0, 1.0))*light, clamp(rd, 0.0, 1.0));
    ...
    return Render(res, clamp(lightEnergy, mix(0.3, 0.1, clamp(max(rain*4, totalRain*0.15), 0, 1)), clamp(density, 0, 1))*max(light, moonLight)*clamp(density, 0, 1)*rv*rv, lightningEnergy, fireEnergy, depth);
}
...
void main(){
    ...
    Render renderOut = render(uv + uvOffset, sceneDistance);
    vec4 col = mix(mix(mix(renderOut.col, vec4(lightingColor, renderOut.col.a), renderOut.lightEnergy*renderOut.col.a), vec4(1, 0.4, 0, renderOut.col.a), renderOut.fireEnergy), vec4(vec3(0.969, 0.855, 1) * 2.0, renderOut.col.a), renderOut.lightningEnergy);
